...
const EventCard = ({ event }: EventCardProps) => {
  ...
  const getDisplayPrice = () => {
    if (event.departures && event.departures.length > 0) {
      const selectedDep = event.departures.find(d => d.isSelected);
      const dep = selectedDep || event.departures[0];
      return {
        discounted: dep.discountedPrice && dep.discountedPrice > 0 ? dep.discountedPrice : undefined,
        original: dep.price || 0
      };
    }
    return {
      discounted: event.discountedPrice && event.discountedPrice > 0 && event.discountedPrice < event.price ? event.discountedPrice : undefined,
          ...
          <div className="flex flex-col space-y-1.5 mb-2.5 text-sm md:text-base text-gray-600">
            ...
            <div className="flex items-baseline gap-2 md:gap-3">
              {priceInfo.discounted && (
                <span className="text-xs md:text-sm text-gray-500 line-through">₹{priceInfo.original.toLocaleString()}</span>
              )}
              <span className="text-lg md:text-xl font-bold text-red-600">₹{priceInfo.discounted ? priceInfo.discounted.toLocaleString() : priceInfo.original.toLocaleString()}</span>
          ...
          <div className="mt-2 space-y-1 text-xs md:text-sm text-gray-600">
            {event.season && event.season.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                <span>{Array.isArray(event.season) ? event.season.join(', ') : event.season}</span>
              </div>
            )}
            {event.ageLimit && (
              <div className="flex items-center gap-2">
                <span className="font-medium">Age Limit:</span>
                <span>{event.ageLimit.min}-{event.ageLimit.max} years</span>
              </div>
            )}
          </div>
        </div>
